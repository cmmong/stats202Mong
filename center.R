...
train_sample = centerdf[train_sample,]
test_sample = centerdf[test_sample,]

# Full Model (Linear)
m = lm( Outcome ~ ., data=centerdf )
...
# Ridge regression: 
#Y = centerdf$Outcome 
model_matrix = model.matrix( Outcome ~ ., data=centerdf )
cv.out = cv.glmnet( model_matrix, centerdf$Outcome, alpha=0 )
plot(cv.out) 
lam_best = cv.out$lambda.1se
#CV Ridge Regression best value of lambda:
print(lam_best)

ridge_model = glmnet( model_matrix, centerdf$Outcome, alpha=0 )
...
# The Lasso: 
cv.out = cv.glmnet( model_matrix, centerdf$Outcome, alpha=1 )
plot( cv.out ) 
lam_best = cv.out$lambda.1se
#Lasso CV best lambda value
print( lam_best )

lasso_model = glmnet( model_matrix, centerdf$Outcome, alpha=1 )
...
pcr_model = pcr( Outcome ~ ., data=centerdf, scale=TRUE, validation="CV" )
...
pls_model = plsr( Outcome ~ ., data=centerdf, scale=TRUE, validation="CV" )
